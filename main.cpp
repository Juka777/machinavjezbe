...
//Zadatak:
	...
	//posebna metoda za setanje oiba,provjeriti jel valjan oib ili postojeci,te na temelju toga namjestiti oib
	//Napraviti program koji omogucuje izradu novih osoba i ima mogucnost  printanja svih unesenih osoba
...
public:
	...
	bool trysetSSN(std::string ssn)
	{
		...
		if (_socialSecurityNumber != "")
		{
			...
			std::cout << "Persons SSN cannot changed!" << std::endl;
			return true;
		}
		if (ssn.size() !=11)
		{
		...
		for (size_t i = 0; i < ssn.size(); i++)
		{
			...
			{
				std::cout << "Social Security Number is Inavlid ! " << std::endl;
